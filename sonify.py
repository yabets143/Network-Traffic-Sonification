...
# Your specific default Wi-Fi interface GUID
# Example \Device\NPF_{18352E06-6B76-4214-BE6A-A99C5314748F}
...
class SoundManager:
    ...
    def play_protocol_sound(self, protocol, packet_size=None):
        ...
        if packet_size:
            frequency = frequency + (packet_size / 200)  # More subtle modulation
        ...
        try:
            ...
            if key == 'q':
                ...
                for proto, count in current_stats['protocols'].items():
                    percentage = (count / current_stats['total_packets'] * 100) if current_stats['total_packets'] > 0 else 0
...
Examples:
  ...
  %(prog)s -i "Ethernet"               # Use specific interface name
    ...
    parser.add_argument("--max-pps", type=int, default=20,
                        help="Maximum packets per second to sonify (prevents audio overload)")
    ...
    handler = lambda pkt: packet_handler(pkt, protocol_identifier, sound_manager, stats, verbose)
    ...
    if stats.packet_count > 0:
        ...
        if final_stats['elapsed_time'] > 0:
            print(f"Average PPS: {final_stats['total_packets']/final_stats['elapsed_time']:.1f}")
        print("\nProtocol breakdown:")
        for proto, count in sorted(final_stats['protocols'].items(), key=lambda x: x[1], reverse=True):
